# Script to make API calls to Rapid7, this one only has asset tagging API payloads, but the API call portion can effectively be used for any API call, but need to make new payloads
import base64
import urllib3
import logging
import requests
...
# logger module will return HTTP response codes from the console server
def setup_logging():
...
def api_call(method=None, data=None, url=None, ssl_verify=False):
    ...
    if method.lower() == "get":
        ...
        if response.status_code == 201 or response.status_code == 200:
            ...
            # exit if something breaks and give servers response
        else:
...
# request body function to hit the api URI at hxxps://host:port/api/3/tags
